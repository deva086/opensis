...
$userName=  User('USERNAME');
 $link=array();
    ...
    foreach($fetch as $key =>$value)
    {
        ...
        if(in_array($userName,$to_arr) || in_array($userName,$arr_cc) || in_array($userName,$arr_bcc))
        {
    ...
    if($total_count>0)
     $to_user_id=implode(',',$id);
        ...
        $to_user_id='null';
$inbox="select * from msg_inbox where mail_id in($to_user_id) and FIND_IN_SET('$userName', mail_read_unread ) =0";
$inbox_info=DBGet(DBQuery($inbox));
if(count($inbox_info)>1)
{
    echo "<p><font color=#FF0000><b>You have ".count($inbox_info)." unread messages</b></font></p>";
}
 else 
{
    if(count($inbox_info)==1)
