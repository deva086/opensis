...
//$header .= '<TD>' . TextInput($RET['TITLE'],'tables['.$_REQUEST['assignment_id'].'][TITLE]',($RET['TITLE']?'':'<FONT color=red>').'Title'.($RET['TITLE']?'':'</FONT>'),'size=36') . '</TD>';
		...
		$header .= '<TD valign=top>' . DateInputAY($new && Preferences('DEFAULT_DUE','Gradebook')=='Y'?DBDate():$RET['DUE_DATE'],'tables['.$_REQUEST['assignment_id'].'][DUE_DATE]',2) . ($_REQUEST['assignment_id']=='new'?'<br><small><font color="gray">Due<font color="gray"></font></font></small>':'<small><font color="gray">Assigned Date<font color="gray"></font></font></small>').'</TD>';
