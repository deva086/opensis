	...
	if(count($RET))
	{
		...
		foreach($RET as $student)
		{
			...
			if($sum_percent>0)
				...
				$sum_points = 0;
                        if($flag)
                            $link['add']['html'] = array('TITLE'=>'<B>Total</B>','LETTER_GRADE'=>'( '.$total_stpoints.' / '.$total_asgnpoints.' ) '._makeLetterGrade($total_stpoints/$total_asgnpoints,"",User('STAFF_ID'),"%").'%&nbsp;'._makeLetterGrade($sum_points,"",User('STAFF_ID')));
			else
			...
			if($value!='-1')
				if($THIS_RET['DUE'] && $value=='')
                                ...
                                else if($THIS_RET['DUE'] || $value!='')
                                {
                                    ...
                                    $per = round($value/$THIS_RET['TOTAL_POINTS'],2);
