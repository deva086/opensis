...
if($_REQUEST['modname'] && $_REQUEST['stu_id'] && $_REQUEST['include_a']=='EnrollmentInfoInc')
{
    ...
    if($res[1]['CALENDER_ID']=='' || $res[1]['CALENDER_ID']==NULL)
    {
    $sid=$_REQUEST['stu_id'];
...
//                $_REQUEST['include']='GeneralInfoInc';
//                echo "<script>window.location.href='Modules.php?modname=students/Student.php&include=GeneralInfoInc&student_id=new&err_msg=true'</script>";
//                
//            }
            else
                $day_valid=true;
...
//				echo $sql;exit;
//                                $qry="SELECT COUNT(1) AS COUNT FROM students s,student_enrollment se WHERE s.BIRTHDATE='".$stud_rec[1]['BIRTHDATE']."'
//        AND s.FIRST_NAME='".$students[1]['FIRST_NAME']."'
//        AND s.LAST_NAME='".$students[1]['LAST_NAME']."' 
//        AND se.GRADE_ID= AND se.SYEAR='".UserSyear()."' AND s.STUDENT_ID=se.STUDENT_ID";
//                                                                  $qry1="SELECT COUNT(1) AS COUNT FROM students s,student_enrollment se WHERE s.BIRTHDATE='".$_REQUEST['students']['BIRTHDATE']."'
//        AND s.FIRST_NAME='".$_REQUEST['students']['FIRST_NAME']."'
//        AND s.LAST_NAME='".$_REQUEST['students']['LAST_NAME']."'
//        AND se.GRADE_ID='".$_REQUEST['values']['student_enrollment']['new']['GRADE_ID']."' AND se.SYEAR='".UserSyear()."' AND s.STUDENT_ID=se.STUDENT_ID";
//    if(isset($_REQUEST['students']['MIDDLE_NAME']))
//    {
//    $stud_rec[1]['MIDDLE_NAME']=str_replace("'","''",$_REQUEST['students']['MIDDLE_NAME']);
//    if($_REQUEST['students']['MIDDLE_NAME']!='')
//    $qry1.=" AND s.MIDDLE_NAME='".$stud_rec[1]['MIDDLE_NAME']."'";
//    }    
//    
//    $qry_exec1=DBGet(DBQuery($qry1));
//    unset($qry1);
////  ECHO $qry_exec1[1]['COUNT'];  exit;
//    if($qry_exec1[1]['COUNT']>0)
//        { }
...
//        print_r($new_tabs);
   ...
   {
       //echo "<br>eta holo ".$_REQUEST['category_id']."<br>.".$_openSIS['selected_tab']."<br>student id=".$_REQUEST['student_id'];
       if($_REQUEST['student_id']!='new')
       {
