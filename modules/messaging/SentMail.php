...
if(isset($_REQUEST['modfunc']) && $_REQUEST['modfunc']=='trash' )
{
    ...
    echo "<script>window.location='Modules.php?modname=messaging/SentMail.php'</script>";
    }
    }
    else
    {
...
$link=array();
$extra=array();
$outbox="SELECT CASE WHEN to_cc = '' THEN to_user ELSE concat( to_user, ' ', to_cc ) END AS TO1, msg_outbox.* FROM msg_outbox where from_user='$userName' AND istrash=0 order by(mail_id) desc";
