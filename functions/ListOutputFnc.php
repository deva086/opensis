...
//                                              
		...
		if($_REQUEST['LO_save']=='1')
		{
			...
			foreach($result as $item)
                        $output.='<tr><td>'.strip_tags($item['TITLE']).'</td><td>'.$item['PERIOD_PULLDOWN'].'</td><td>'.$item['ROOM'].'</td><td>'.$item['COURSE_MARKING_PERIOD_ID'].'</td><td>'.$item['START_DATE'].'</td><td>'.($item['END_DATE']==0?'':$item['END_DATE']).'</td></tr>';
			$output .= "</table>";
			header("Cache-Control: public");
			header("Pragma: ");
			header("Content-Type: application/$extension");
